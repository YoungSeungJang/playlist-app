...
const ArtistsPage: React.FC = () => {
  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      setIsSearching(true)
      try {
        const apiUrl = `http://localhost:3001/api/spotify/search?q=${encodeURIComponent(debouncedSearchTerm)}&type=artist&limit=20`
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4">
        ...
        {/* 검색 결과가 없는 경우 */}
        {!isSearching && debouncedSearchTerm.length >= 2 && artists.length === 0 && (
          <div className="text-center py-16">
            <MagnifyingGlassIcon className="mx-auto h-16 w-16 text-gray-400 mb-4" />
            <h3 className="text-xl font-medium text-gray-900 mb-2">
              "{debouncedSearchTerm}"에 대한 아티스트 검색 결과가 없습니다
            </h3>
